...
namespace DTcms.Common
{
    public class Utils
    {
        ...
        /// <param name="defValue">缺省值</param>
        /// <returns>转换后的bool类型结果</returns>
        public static bool StrToBool(string expression, bool defValue = false)
        {
            if (expression != null)
            {
        ...
        public static int ObjToInt(object expression, int defValue = 0)
        {
            if (expression != null)
        ...
        public static decimal ObjToDecimal(object expression, decimal defValue = 0)
        {
            if (expression != null)
        ...
        /// <param name="defValue">缺省值</param>
        /// <returns>转换后的decimal类型结果</returns>
        public static decimal StrToDecimal(string expression, decimal defValue = 0)
        {
        ...
        /// <param name="defValue">缺省值</param>
        /// <returns>转换后的int类型结果</returns>
        public static float ObjToFloat(object expression, float defValue = 0)
        {
            if (expression != null)
        ...
        /// <param name="defValue">缺省值</param>
        /// <returns>转换后的int类型结果</returns>
        public static float StrToFloat(string expression, float defValue = 0)
        {
            if ((expression == null) || (expression.Length > 10))
                return defValue;
        ...
        public static string GetOrderNumber()
        {
            lock (Locker)  //lock 关键字可确保当一个线程位于代码的临界区时，另一个线程不会进入该临界区。
            {
                ...
                Thread.Sleep(100);
                return DateTime.Now.ToString("yyyyMMddHHmmssfff") + _sn.ToString();
            }
        } 
        ...
        public static void DeleteUpFile(string _filepath)
        {
            ...
            {
                string thumbnailpath = _filepath.Substring(0, _filepath.LastIndexOf("/") + 1) + "mall_" + _filepath.Substring(_filepath.LastIndexOf("/") + 1);
        ...
        public static string ConvertMinutesToHours(int minutes)
        {
            ...
            return (minutes/60) + "小时" + (minutes - ((minutes/60))) + "分钟";
        }
        ...
        #region 生成、还原短链接
        /// <summary>
        /// 通过百度短链接API生成短链接，返回原来的链接说明出错  
        /// </summary>
        /// <param name="url">长连接</param>
        /// <returns></returns>
        public static string getShortUrl(string url)
        {
            string jsonResult = HttpPost("http://dwz.cn/create.php", "url=" + url);
            CommonJsonModel json = new CommonJsonModel(jsonResult);
            if (json.GetValue("status") == "0")
            {
                return json.GetValue("tinyurl").Replace("\\","");
            }
            return url;
        }
        /// <summary>
        /// 通过百度短链接API查询长链接，返回空则说明出错   
        /// </summary>
        /// <param name="url">短链接</param>
        /// <returns></returns>
        public static string getLongUrl(string url)
        {
            string jsonResult = HttpPost("http://dwz.cn/query.php", "tinyurl=" + url);
            CommonJsonModel json = new CommonJsonModel(jsonResult);
            if (json.GetValue("status") == "0")
            {
                return json.GetValue("longurl").Replace("\\", "");
            }
            return string.Empty;
        }
        #endregion
